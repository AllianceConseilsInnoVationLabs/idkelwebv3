...
export default function AuthWrapperOne({
  ...
  return (
    <>
      <Link
        href={'/'}
        className="hidden sticky start-0 top-0 z-20 flex items-center justify-center bg-blue p-3.5 text-sm font-medium text-white md:p-4 lg:hidden"
      >
        <PiArrowLeftBold />
        <Text className="ms-1 font-lexend">Back to home</Text>
      </Link>

      <div className="min-h-screen justify-between gap-x-8 px-4 py-8 pt-0 md:pt-12 lg:flex lg:p-0 xl:gap-x-10 xl:p-7 2xl:p-0 2xl:pt-0 [&>div]:min-h-[calc(100vh-80px)]">
        <div className="relative flex w-full items-center justify-center lg:w-5/12 2xl:justify-end 2xl:pe-24">
          <div className=" w-full max-w-sm md:max-w-md lg:py-7 lg:ps-3 lg:pt-16 2xl:w-[630px] 2xl:max-w-none 2xl:ps-20 2xl:pt-7">
            <div className="mb-7 px-6 pt-3 text-center md:pt-0 lg:px-0 lg:text-start xl:mb-8 2xl:mb-10">
              ...
              >
                <Image src={logoImg} alt="idkel Logo" />
              </Link>
              <Title
                as="h2"
              ...
              <Text className="hidden leading-[1.85] text-gray-700 md:leading-loose lg:pe-8 2xl:pe-14">
                {description}
              </Text>
            </div>
            {isSocialLoginActive && (
        ...
        <div className="hidden w-7/12 items-center justify-center rounded-[20px] bg-gray-50 lg:flex dark:bg-gray-100/40">
            {pageImage}
        </div>
      </div>
    </>
