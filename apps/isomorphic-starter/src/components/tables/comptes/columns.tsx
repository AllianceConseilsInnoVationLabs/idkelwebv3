...
import { formatMillier, formatTel } from '@/lib/utils';
...
export const getColumns = ({
  ...
  },
  {
    ...
    render: (libelle: string, row: any) => (
      <>
        ...
        <Text className="text-[11px] flex items-center gap-x-2 font-semibold">{row.provenance} <ArrowRight size={12} /> {row.destination}</Text>
      </>
    ),
  },
  {
    ...
    render: (montant: number, row: any) => (
      <>
        ...
        <Text className="text-sm">Solde : { formatMillier(row.nouveau_solde) } </Text>
      </>
    ),
  },
  {
    title: <HeaderCell title="Destinataire" />,
    dataIndex: 'destination',
    key: 'destination',
    width: 260,
    render: (destination: string, row: any) => (
      <Text>{ formatTel(destination) }</Text>
    ),
  },
...

  {
    ...
    onHeaderCell: () => onHeaderCellClick('statut'),
    dataIndex: 'statut',
    key: 'statut',
    width: 180,
    render: (statut: string) => {
      return (
        statut === 'Effectué' ? (
          <Badge color="success">{ statut }</Badge>
        ) : (
          <Badge color="warning">{ statut }</Badge>
        )
      );
    },
  },
