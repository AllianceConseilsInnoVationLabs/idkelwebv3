...
import { passwordFormSchema, PasswordFormTypes } from "@/validators/password-settings.schema";
  ...
  return (
    <>
      <Form<PasswordFormTypes>
        validationSchema={passwordFormSchema(t)}
        resetValues={reset}
        onSubmit={onSubmit}
        className="@container"
        useFormProps={{
                  ...
                  <Controller
                    ...
                    render={({ field: { onChange, value } }) => (
                      <Password
                        placeholder={t("form-password-placeholder")}
                        helperText={
                          getValues().newPassword.length < 8 && t("form-password-helper-text")
                        }
                        onChange={onChange}
      ...
      <div className="flex items-center gap-6 border-b border-dashed border-muted py-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <Text className="text-sm text-gray-500">{t("text-country-address")}</Text>
            <span className="h-1 w-1 rounded-full bg-gray-600" />
            <Text className="text-sm text-gray-500">{t("text-logged-in-time")}</Text>
          </div>
          <Text
            as="span"
